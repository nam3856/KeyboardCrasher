...
{
	[System.Serializable]
	public class GameRichPresenceUser
	{
		/// <summary>
        ...
        [SerializeField] int _discriminator;
		...
		public delegate void AvatarDownloadCallback(GameRichPresenceUser user, Texture2D avatar);
		...
		public IEnumerator GetAvatarCoroutine(DiscordAvatarSize size = DiscordAvatarSize.x128, AvatarDownloadCallback callback = null)
		{
			...
			else
			{
				...
				if (CacheLevel != CacheLevelFlag.None)
				{
					...
					string filename = string.Format(format + ".{3}", ID, avatarHash, size.ToString(), GameRichPresenceUser.AvatarFormat.ToString().ToLowerInvariant());
				...
				{
                    using (UnityWebRequest req = UnityWebRequestTexture.GetTexture(GetAvatarURL(GameRichPresenceUser.AvatarFormat, size)))
                    {
		...
		void CacheAvatarTexture(Texture2D texture, string path)
		{
			...
			if (CacheLevel != CacheLevelFlag.None)
			{
				...
				//Encode the image
				byte[] bytes;
				switch (GameRichPresenceUser.AvatarFormat)
				{
					default:
		...
		/// </summary>
		static void SetupDefaultCacheDirectory()
		{
		...
		string GetAvatarURL(DiscordAvatarFormat format, DiscordAvatarSize size)
		{
		...
		/// <summary>
		/// Implicit casting from a DiscordRPC.User to a GameRichPresenceUser
		/// </summary>
		/// <param name="user"></param>
		public static implicit operator GameRichPresenceUser(User user) { return new GameRichPresenceUser(user); }
	}
	...
	public static class DiscordUserExtension
	{
		/// <summary>
		/// Gets the user avatar as a Texture2D and starts it with the supplied monobehaviour. It will first check the cache if the image exists, if it does it will return the image. Otherwise it will download the image from Discord and store it in the cache, calling the callback once done.
		/// <para>An alias of <see cref="GameRichPresenceUser.CacheAvatar(MonoBehaviour, DiscordAvatarSize, AvatarDownloadCallback)"/> and will return the new <see cref="GameRichPresenceUser"/> instance.</para>
		/// </summary>
		/// <param name="coroutineCaller">The target object that will start the coroutine</param>
		/// <param name="size">The target size of the avatar. Default is 128x128</param>
		/// <param name="callback">The callback for when the texture completes. Default is no-callback, but its highly recommended to use a callback</param>
		/// <returns>Returns the generated <see cref="GameRichPresenceUser"/> for this <see cref="DiscordRPC.User"/> object.</returns>
		public static GameRichPresenceUser GetAvatar(this User user, MonoBehaviour coroutineCaller, DiscordAvatarSize size = DiscordAvatarSize.x128, GameRichPresenceUser.AvatarDownloadCallback callback = null)
		{
			var du = new GameRichPresenceUser(user);
		...
		/// <summary>
		/// Gets the user avatar as a Texture2D as a enumerator. It will first check the cache if the image exists, if it does it will return the image. Otherwise it will download the image from Discord and store it in the cache, calling the callback once done.
		/// <para>An alias of <see cref="GameRichPresenceUser.CacheAvatarCoroutine(DiscordAvatarSize, GameRichPresenceUser.AvatarDownloadCallback)"/> and will return the new <see cref="GameRichPresenceUser"/> instance in the callback.</para>
		/// </summary>
		/// <param name="size">The target size of the avatar. Default is 128x128</param>
		/// <param name="callback">The callback for when the texture completes. Default is no-callback, but its highly recommended to use a callback</param>
		/// <returns></returns>
		public static IEnumerator GetAvatarCoroutine(this User user, DiscordAvatarSize size = DiscordAvatarSize.x128, GameRichPresenceUser.AvatarDownloadCallback callback = null)
		{
			var du = new GameRichPresenceUser(user);
		...
		/// <summary>
		/// Gets the default avatar for the given user. Will check the cache first, and if none are available it will then download the default from discord.
		/// <para>An alias of <see cref="GameRichPresenceUser.CacheDefaultAvatarCoroutine(DiscordAvatarSize, GameRichPresenceUser.AvatarDownloadCallback)"/> and will return the new <see cref="GameRichPresenceUser"/> instance in the callback.</para>
		/// </summary>
		/// <param name="size">The size of the target avatar</param>
		/// <param name="callback">The callback that will be made when the picture finishes downloading.</param>
		/// <returns></returns>
		public static IEnumerator GetDefaultAvatarCoroutine(this User user, DiscordAvatarSize size = DiscordAvatarSize.x128, GameRichPresenceUser.AvatarDownloadCallback callback = null)
		{
			var du = new GameRichPresenceUser(user);
			return du.GetDefaultAvatarCoroutine(size, callback);
		}
